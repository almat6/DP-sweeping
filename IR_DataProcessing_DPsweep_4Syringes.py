...
def CalConversion(x:float, y:float)->float:
    ...
    y: initial concentration of Monomer
    '''
...
# create a empty csv file to save all the calculated data
...
V_dead = 0.2629 #float(input('\033[1;31mPlease input the dead Volume of your reactor (ml):>> \033[0m'))
V_sample = 0.2872 #float(input('Please input the Volume between the sample collector and equipment (ml):>> '))
V_input = 0.1705 #float(input('Please input the Volume between mixer and the reactor (ml):>> '))
StockCon_Monomer = 4 #float(input('\033[1;31mPlease input the initial concentration of your Monomer(M):>> \033[0m'))
StockCon_RAFTAgent = 0.4 #float(input('\033[1;31mPlease input the initial concentration of your RaftAgent (M):>> \033[0m'))
StockCon_Initiator = 0.02

# print(f'V_reactor is {V_reactor} ml,\nV_dead is {V_dead} ml,\nC_monomer is {C_monomer} M,\nC_RaftAgent is {C_RaftAgent} M, \nC_Initiator is {C_Initiator} M')
Pro_Monomer = 4 #float(input('\033[1;31mPlease input the proposed  concentration of Monomer:>> \033[0m'))
...
FlowRateInitiator = []
DegP = []
####### calculate the flowrate of Initiator
...
FRInitiator = (StockCon_Monomer * FRMonomer / 1000) / StockCon_Initiator # 1000 is the ratio between monomer and initiator, this should be 2000 according to the RAFT multiblock paper however 1000 is okay for big monomers

for i in range(No_steps+1):
    dp = DP_Monomerstart + i * DPDecreaseStep
    FRRaftAgent = (StockCon_Monomer * FRMonomer / dp) / StockCon_RAFTAgent #the ratio between monomer and RAFT agent is the DP
...
Reactionparameter = [
                    ...
                    f"The initial concentration of raftgent is {StockCon_Initiator} M",
...
class Handler(FileSystemEventHandler):
    def on_created(self, event):
        ...
        if len(listfile_created)*5 >= (V_dead+V_input+V_reactor)*60/Flowrate + 240:
            try:
